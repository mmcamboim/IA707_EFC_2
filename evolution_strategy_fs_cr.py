...
class EvolutionStrategyFSCR:
    ...
    def popInit(self,pop_size):
        ...
        self.__pop_fitness_share = np.zeros(pop_size)
    ...
    def runEE(self,pop_size,var_std,cov_std,phi_s,phi_m,generations):
        # Begin Variables
        ...
        # 2. Get Fitness
        self.__pop_fitness,self.__pop_fitness_share = self.getPopFitness(self.population,phi_s)
        ...
        for generation in range(generations):
            ...
            # Selection
            pop_selected = self.randomSelection(pop_size,pop_size*2,phi_m)
            # Crossover
            pop_son = self.crossoverForRealVector(pop_selected)
            # Mutation
            pop_son = self.mutationForRealVector(pop_son,var_std,cov_std)
            pop_son_fitness,pop_son_fitness_share = self.getPopFitness(pop_son,phi_s)
            # Elitism
            self.elitism(pop_son, pop_son_fitness_share)
            self.__pop_fitness,self.__pop_fitness_share = self.getPopFitness(self.population,phi_s)
    ...
    def sharingFunction(self,distance,phi_s):
        if distance >= phi_s:
            sh = 0.0
        else:
            sh = 1.0 - (distance/phi_s)**2.0
        return sh
    
    def getFitnessShare(self,pop,this_individual_idx,this_individual,phi_s):
        ci = 0.0
        pop_size = pop.shape[0]
        for other_individual_idx in range(pop_size):
            if this_individual_idx == other_individual_idx:
                continue
            other_individual = pop[other_individual_idx,:]
            distance = getEuclideanDistance(this_individual,other_individual)
            ci = ci + self.sharingFunction(distance,phi_s)
        if ci == 0:
            ci = 1.0
        return ci
            
    def getPopFitness(self,pop,phi_s):
        pop_size = pop.shape[0]
        pop_fitness = np.zeros(pop_size)
        pop_fitness_share = np.zeros(pop_size)
        for this_individual_idx in range(pop_size):
            ...
            ci = self.getFitnessShare(pop,this_individual_idx,pop[this_individual_idx,:],phi_s)
            pop_fitness_share[this_individual_idx] = pop_fitness[this_individual_idx] / ci
        return pop_fitness,pop_fitness_share
    
    # Selection ==============================================================
    def randomSelection(self,pop_size,individuals_to_be_selected,phi_m):
        ...
        for pop_selected_idx in range(individuals_to_be_selected):
            if(pop_selected_idx % 2 == 0):
                ...
                pop_selected[pop_selected_idx,:] = self.speaction(selected_individual_idx,last_individual,pop_size,phi_m)
        return pop_selected
    
    def speaction(self,last_individual_idx,last_individual,pop_size,phi_m):
    ...
    def elitism(self,pop_son,pop_son_fitness):
        ...
        all_individuals_fitness[:pop_size] = np.copy(self.fitness_share)
    ...
    def fitness_share(self):
        return self.__pop_fitness_share
    ...
    def best_solution(self):
        best_solution_idx = np.argmax(self.__pop_fitness)
        return self.population[best_solution_idx,0:2]
